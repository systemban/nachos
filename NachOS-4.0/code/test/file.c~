...
/*整数转字符串*/ void itoa(int n, char* line); 
 
/*将数字写入文件*/ void writeNum(int num,int fid); 
 
int main(){     int result,resultReadF;              
int fid1, fid2, fid3;     char str[64];  
//测试Create和Open     
...
result = Close(fid1);     //将返回值写入result.txt文件     
writeNum(result,fid2); 
...
void itoa(int n, char* line){     
  int i=0;     int j=0;     int neg = -1;    
  if(n<0) {         n=-n;         neg=1;     }    
  while(n){         line[i++] = '0'+n%10;         n/=10;     }     
  if(neg==1) line[i++] = '-';     
  for(i=i-1;i>j;j++,i--){         char t = line[i];         line[i] = line[j];         line[j] = t;     } } 
  
  void writeNum(int num,int fid)
  {     int cnt = 0;     int t = num;     char numStr[64]; 
